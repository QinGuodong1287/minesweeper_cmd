...
class Minesweeper(tutorial.GameTutorialMixin, graphics.Window):
    def __init__(self):
        super().__init__()
        self.welcome_msg = localize.tr("""欢迎来到扫雷。
按上下方向键选择操作，按Enter键执行所选操作。""")
    ...
    def reset_options(self):
        self.options.clear()
        self.options.append({"name": localize.tr("开始新游戏"),
                             "command": lambda: self.main_game(False)})
        if os.path.exists(map_file):
            self.options.append({"name": localize.tr("继续游戏"),
                                 "command": self.main_game})
        self.options.append({"name": localize.tr("打开游戏设置"),
    ...
    def eventloop(self):
        ...
        self.win.addstr(terminal_size.lines - 1, 0, __version__,
                        curses.A_NORMAL)
        welcome_msg_lines = list(self.welcome_msg.splitlines())
        option_len = len(self.options)
        start_y = ((terminal_size.lines - option_len - len(welcome_msg_lines))
                   // 2)
        ...
        for index, option in enumerate(self.options):
            self.win.addstr(
                start_y + len(welcome_msg_lines) + index, left_x,
                str_ljust("{}.{}".format(index + 1, option["name"]),
                          option_name_len),
                curses.A_REVERSE
...
class InitialSettings(graphics.Window):
    ...
    def eventloop(self):
        keyset_prompt = localize.tr("""为了更好地畅玩扫雷，建议你查看键位设置。
是否现在打开键位设置？(y/n)""").splitlines()
        keyset_bad_msg = localize.tr("你的选择错误。请按“y”或“n”。")
        start_y = (self.terminal_size.lines - len(keyset_prompt) - 1) // 2
        for line_num, line in enumerate(keyset_prompt):
            self.win.addstr(start_y + line_num,
                            (self.terminal_size.columns - uni_len(line)) // 2,
        ...
        while True:
            if time() - self.bad_time > 3:
                self.win.addstr(
                    start_y + len(keyset_prompt),
                    ((self.terminal_size.columns - uni_len(keyset_bad_msg))
                     // 2),
                    ' ' * uni_len(keyset_bad_msg), curses.A_NORMAL)
            key = self.win.getch(
                start_y + len(keyset_prompt) - 1,
                ((self.terminal_size.columns - uni_len(keyset_prompt[-1])) // 2
                 + uni_len(keyset_prompt[-1])))
            ...
            if key > -1:
                self.win.addstr(
                    ...
                    ((self.terminal_size.columns - uni_len(keyset_bad_msg))
                     // 2),
                    keyset_bad_msg, curses.A_NORMAL)
                self.bad_time = time()
        curses.noecho()
        if key == ord('y'):
