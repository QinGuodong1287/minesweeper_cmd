...
class GameTutorialMixin:
    ...
    def reg_tutor(self, name, text, x=inf, y=inf, attr=None):
        ...
        if name in self.tutor_table:
            ...
            self.tutor_table[name] = {"text": text, "x": x, "y": y,
                                      "attr": attr}
    ...
    def switch_tutor(self, name, win=None):
        ...
        text_lines = (str(text() if text and callable(text) else text)
                      .splitlines())
        text_max_line_len = max(reduce(
            lambda length, line: max(length, uni_len(line)), text_lines, 0),
                                uni_len(GameTutorialMixin.title))
        start_y = min(tutor_item["y"],
                      terminal_size.lines - len(text_lines) - 3)
        start_x = min(tutor_item["x"],
                      terminal_size.columns - text_max_line_len - 3)
        win.addstr(start_y, start_x, '+' + '-' * text_max_line_len + '+',
                   curses.A_NORMAL)
        win.addstr(start_y + 1, start_x, '|', curses.A_NORMAL)
        win.addstr(start_y + 1, start_x + 1,
                   str_ljust(GameTutorialMixin.title, text_max_line_len),
                   tutor_item["attr"])
        win.addstr(start_y + 1, start_x + 1 + text_max_line_len, '|',
                   curses.A_NORMAL)
        for line_num, line in enumerate(text_lines, 1):
            win.addstr(start_y + 1 + line_num, start_x, '|', curses.A_NORMAL)
            win.addstr(start_y + 1 + line_num, start_x + 1,
                       str_ljust(line, text_max_line_len), tutor_item["attr"])
            win.addstr(start_y + 1 + line_num, start_x + 1 + text_max_line_len,
                       '|', curses.A_NORMAL)
        win.addstr(start_y + 2 + len(text_lines), start_x,
                   '+' + '-' * text_max_line_len + '+', curses.A_NORMAL)
...
class GameTutorialPage(graphics.Window):
    ...
    def eventloop(self):
        global tutor_active
        self.win.addstr(0, 0, localize.tr("游戏教程"), curses.A_NORMAL)
        self.win.addstr(1, 0, localize.tr("欢迎来到扫雷游戏教程。按Enter键进入教程，按“q”键退出教程。"),
                        curses.A_NORMAL)
